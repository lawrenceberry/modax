"""Warp-divergence benchmark: Rodas5 on a coupled van der Pol lattice.

This benchmark measures wall time and wasted lane work when stiff 70D
coupled van der Pol trajectories with varied initial conditions share a
Rodas5 while-loop batch. All cases use identical stiffness parameters.

Usage:
    uv run python scripts/5_rodas5_divergence/main.py
"""
...
_PARAMS = coupled_vdp_lattice.make_params(_N_TRAJ)
...
_SCENARIOS = (Scenario("ic_large", "large y0", "#e02b2b"),)
...
def make_initial_conditions(scenario: Scenario, size: int) -> np.ndarray:
    return coupled_vdp_lattice.make_initial_conditions(scenario.key, size)
...
def plot(rows: list[dict], gpu_name: str, output_path: Path) -> None:
    ...
    ax_time.set_title(f"Rodas5 divergence - coupled VDP lattice - {gpu_name}")
