"""Rodas5 ensemble ODE solver for linear systems dy/dt = M(p) y.

The system is fully defined by the Jacobian jac_fn(y, p) -> J, where
J[i][j] = M(p)[i][j]. F(y, p) = J @ y is computed internally, so no
separate ode_fn is needed.

API::

    solve = make_solver(jac_fn)
    results = solve(y0_batch, t_span, params_batch)
"""
...
def _make_rodas5_step(jac_fn, n_vars, n_params):
    nv = n_vars
    np_ = n_params
    ...
    def _eval_F(src_ref, p_ref, dst_ref):
        """dst = J(y, p) @ y  (F for linear ODE dy/dt = M(p) y)."""
        y_t = tuple(src_ref.at[:, i][...] for i in range(nv))
        p_t = tuple(p_ref.at[:, i][...] for i in range(np_))
        J = jac_fn(y_t, p_t)

        def eval_row(i, carry):
            row_i = _tuple_select(J, i)

            def eval_col(j, acc):
                return acc + _tuple_select(row_i, j) * _tuple_select(y_t, j)

            fi = jax.lax.fori_loop(0, nv, eval_col, jnp.zeros_like(y_t[0]))
            dst_ref.at[:, pl.ds(i, 1)][...] = fi[:, None]
            return carry

        jax.lax.fori_loop(0, nv, eval_row, jnp.int32(0))
    ...
    def step(p_ref, dt, w_ref, x_ref, y_ref, k_ref, u_ref):
        ...
        # --- Jacobian via user-supplied jac_fn (no JVP tracing) ---
        y_tuple = tuple(y_ref.at[:, i][...] for i in range(nv))
        p_tuple = tuple(p_ref.at[:, i][...] for i in range(np_))

        # J[i][j] = df_i/dy_j, returned as nv-tuple of nv-tuples
        J = jac_fn(y_tuple, p_tuple)

        # Fill W = I/(dt*gamma) - J into w_ref.
        def fill_w_row(i_s, carry):
            row_i = _tuple_select(J, i_s)

            def fill_w_col(j_s, carry):
                idx = i_s * nv + j_s
                val = -_tuple_select(row_i, j_s)
                val = jnp.where(i_s == j_s, val + d, val)
                w_ref.at[:, pl.ds(idx, 1)][...] = jnp.reshape(val, (-1, 1))
                return carry

            return jax.lax.fori_loop(0, nv, fill_w_col, carry)

        jax.lax.fori_loop(0, nv, fill_w_row, jnp.int32(0))
...
def make_solver(jac_fn):
    """Create a Pallas ensemble Rodas5 solver for dy/dt = M(p) y.

    Args:
        jac_fn: ``(y, p) -> J`` where J is an nv-tuple of nv-tuples with
            ``J[i][j] = M(p)[i][j]``. Both y and p are tuples of
            per-trajectory scalar-like JAX values.

    Returns:
        ``solve_ensemble_pallas(y0_batch, t_span, params_batch, ...)``
    """
    ...
    def _rodas5_pallas_solve(
        ...
        *,
        ...
        step_fn = _make_rodas5_step(jac_fn, n_vars, n_params)
        nv = n_vars
        ...
        def kernel_body(params_ref, y0_ref, y_ref, w_ref, x_ref, k_ref, u_ref):
            ...
            def body_fn(state):
                ...
                step_fn(params_ref, dt_use, w_ref, x_ref, y_ref, k_ref, u_ref)
    ...
    def solve_ensemble_pallas(
        ...
        *,
        ...
        """Solve ODE ensemble using Rodas5 with explicit Jacobian.
